...
void _usbd_reset(usbd_device *usbd_dev)
{
	...
	usbd_ep_setup(usbd_dev, 0, USB_ENDPOINT_ATTR_CONTROL, 64, 64, NULL);
...
{
	usbd_dev->driver->ep_setup(usbd_dev, addr, type, max_size, fifoSize, callback);
}

uint16_t usbd_ep_write_packet(usbd_device *usbd_dev, uint8_t addr,
			 const void *buf, uint16_t len, uint16_t max_packet_len)
{
	return usbd_dev->driver->ep_write_packet(usbd_dev, addr, buf, len, max_packet_len);
}
