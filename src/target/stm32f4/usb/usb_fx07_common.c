...
void stm32fx07_ep_setup(usbd_device *usbd_dev, uint8_t addr, uint8_t type,
			uint16_t max_size, uint16_t fifoSize,
			void (*callback) (usbd_device *usbd_dev, uint8_t ep))
{
	...
	if (dir) {
		REBASE(OTG_DIEPTXF(addr)) = ((fifoSize / 4) << 16) |
					     usbd_dev->fifo_mem_top;
		usbd_dev->fifo_mem_top += fifoSize / 4;
...
uint16_t stm32fx07_ep_write_packet(usbd_device *usbd_dev, uint8_t addr,
			      const void *buf, uint16_t len, uint16_t max_packet_len)
{
...
//		if ((i > 0) && (REBASE(OTG_DIEPCTL(i)) & OTG_FS_DIEPCTL0_EPENA)) {
//			if (REBASE(OTG_DIEPINT(i)) & OTG_FS_DIEPINTX_ITTXFE) {
//				REBASE(OTG_DIEPCTL(i)) |= OTG_FS_DIEPCTL0_EPDIS;
//				while (0 == (REBASE(OTG_DIEPINT(i)) & OTG_FS_DIEPINTX_EPDISD));
//				REBASE(OTG_DIEPCTL(i)) =  OTG_FS_DIEPINTX_EPDISD | OTG_FS_DIEPINTX_TXFE | OTG_FS_DIEPINTX_ITTXFE;
//			}
//		}
	}
