...
#define CREATE_FIFO_PEEK_FCT(fifoType, fifoPrefix, fifoCnt) \
	static fifoPrefix ## _ErrorType_t fifoPrefix ##_peek(const fifoPrefix ##_fifoHandle_t *i_handle,const  uint16_t i_index, fifoType *o_data); \
	static fifoPrefix ## _ErrorType_t fifoPrefix ##_peek(const fifoPrefix ##_fifoHandle_t *i_handle,const  uint16_t i_index, fifoType *o_data) \
	{\
		...
		{ \
			...
			{\
				const uint16_t index = (i_handle->start + i_index) % (fifoCnt);\
...
#define CREATE_FIFO_GET_CNT_FCT(fifoType, fifoPrefix, fifoCnt) \
	static uint16_t fifoPrefix ##_getCount(const fifoPrefix ##_fifoHandle_t *i_handle); \
	static uint16_t fifoPrefix ##_getCount(const fifoPrefix ##_fifoHandle_t *i_handle) \
	{\
		uint16_t ret = 0U;\
		GEN_FIFO_CLI_FUNCTION;\
